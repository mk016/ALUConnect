...
import { Mail, Linkedin, MapPin, Briefcase, GraduationCap, Calendar, Building2, FileText, MessageSquare } from "lucide-react";
import { motion } from "framer-motion";
import alumniData from '@/data/alumni.json'; // Ensure alumniData is correctly imported

export default function AlumniProfile() {
  const { id } = useParams();
  const [messageOpen, setMessageOpen] = useState(false);

  // Find the alumni data based on the ID from the URL params
  const alumni = alumniData.alumni.find((alumni) => alumni.id === parseInt(id));

  // Check if alumni data exists
  if (!alumni) {
